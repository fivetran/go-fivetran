...
func TestNewConnectionFingerprintDetailsMock(t *testing.T) {
	// arrange
	...
	handler := mockClient.When(http.MethodGet, fmt.Sprintf("/v1/connections/%v/fingerprints/%v", testConnectionId, testHash)).ThenCall(
	...
	// act & assert
	response, err := ftClient.NewConnectionFingerprintDetails().
