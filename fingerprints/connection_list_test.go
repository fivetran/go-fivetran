...
func TestNewConnectionFingerprintsListMock(t *testing.T) {
	// arrange
	...
	handler := mockClient.When(http.MethodGet, fmt.Sprintf("/v1/connections/%v/fingerprints", testConnectionId)).ThenCall(
	...
	// act & assert
	response, err := ftClient.NewConnectionFingerprintsList().
