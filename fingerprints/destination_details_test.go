...
func TestNewDestinationFingerprintDetailsMock(t *testing.T) {
	// arrange
	...
	handler := mockClient.When(http.MethodGet, fmt.Sprintf("/v1/destinations/%v/fingerprints/%v", testDestinationId, testHash)).ThenCall(
	...
	// act & assert
	response, err := ftClient.NewDestinationFingerprintDetails().
