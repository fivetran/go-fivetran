...
func TestConnectorSchemaCreateFullMappingMock(t *testing.T) {
	// arrange
	ftClient, mockClient := testutils.CreateTestClient()
	handler := mockClient.When(http.MethodPost, fmt.Sprintf("/v1/connectors/%v/schemas", testutils.TEST_CONNECTOR_ID)).ThenCall(
	...
	svc := ftClient.
		NewConnectorSchemaCreateService().
...
func assertCreateSchemaResponse(t *testing.T, response connectors.ConnectorSchemaDetailsResponse) {
