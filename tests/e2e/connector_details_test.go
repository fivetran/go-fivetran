...
func TestNewConnectorDetailsE2E(t *testing.T) {

	connectorId := testutils.CreateTempConnector(t)
	details, err := testutils.Client.NewConnectorDetails().ConnectorID(connectorId).Do(context.Background())
	...
	testutils.AssertEqual(t, details.Data.ID, connectorId)
