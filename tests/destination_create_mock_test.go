...
func assertRequest(t *testing.T, request map[string]interface{}) {
    ...
    assertKey(t, "storage_account_name", config, STORAGE_ACCOUNT_NAME)
    assertKey(t, "container_name", config, CONTAINER_NAME)
...
func assertResponse(t *testing.T, response destinations.DestinationDetailsWithSetupTestsResponse) {
    ...
    assertEqual(t, response.Data.Config.StorageAccountName, STORAGE_ACCOUNT_NAME)
    assertEqual(t, response.Data.Config.ContainerName, CONTAINER_NAME)
