...
func TestNewConnectorColumnConfigModifyServiceMock(t *testing.T) {
    ...
    handler := mockClient.When(http.MethodPatch, fmt.Sprintf("/v1/connectors/%v/schemas/schema_1/tables/table_1/columns/column_1", TEST_CONNECTOR_ID)).ThenCall(

        func(req *http.Request) (*http.Response, error) {
            body := requestBodyToJson(t, req)
            assertConnectorColumnConfigModifyRequest(t, body)
            response := mock.NewResponse(req, http.StatusOK, prepareConnectorColumnConfigModifyResponse())
            return response, nil
        })

    svc := ftClient.
        NewConnectorColumnConfigModifyService().
        Enabled(true).
        Hashed(false).
    ...
    assertConnectorColumnConfigModifyResponse(t, response)
}

func assertConnectorColumnConfigModifyRequest(t *testing.T, request map[string]interface{}) {
...
func assertConnectorColumnConfigModifyResponse(t *testing.T, response connectors.ConnectorSchemaDetailsResponse) {
    ...
    assertEqual(t, *table1_schema1.SyncMode, "SOFT_DELETE")
...
func prepareConnectorColumnConfigModifyResponse() string {
    return `{
    "code": "Success",
    "data": {
