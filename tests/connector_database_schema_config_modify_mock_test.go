...
func TestConnectorDatabaseSchemaConfigModifyServiceMock(t *testing.T) {
    ...
    handler := mockClient.When(http.MethodPatch, fmt.Sprintf("/v1/connectors/%v/schemas/schema_1", TEST_CONNECTOR_ID)).ThenCall(
...
func assertSchemaModifyRequest(t *testing.T, request map[string]interface{}) {
    ...
    assertKey(t, "enabled", table1, true)
    assertKey(t, "sync_mode", table1, "HISTORY")

    table2 := request["tables"].(map[string]interface{})["table_2"].(map[string]interface{})
    
    assertKey(t, "enabled", table2, false)
