...
func TestConnectionSchemaCreateFullMappingMock(t *testing.T) {
	// arrange
	ftClient, mockClient := testutils.CreateTestClient()
	handler := mockClient.When(http.MethodPost, fmt.Sprintf("/v1/connections/%v/schemas", testutils.TEST_CONNECTION_ID)).ThenCall(

		func(req *http.Request) (*http.Response, error) {
			body := testutils.RequestBodyToJson(t, req)
			assertCreateSchemaRequest(t, body)
			response := mock.NewResponse(req, http.StatusOK, prepareCreateSchemaResponse())
			return response, nil
		})
	...
	svc := ftClient.
		NewConnectionSchemaCreateService().
	...
	assertCreateSchemaResponse(t, response)
}

func assertCreateSchemaRequest(t *testing.T, request map[string]interface{}) {
...
func assertCreateSchemaResponse(t *testing.T, response connections.ConnectionSchemaDetailsResponse) {
