...
func TestNewConnectionTableConfigUpdateServiceMock(t *testing.T) {
	// arrange
	ftClient, mockClient := testutils.CreateTestClient()

	handler := mockClient.When(http.MethodPatch, fmt.Sprintf("/v1/connections/%v/schemas/schema_1/tables/table_1", testutils.TEST_CONNECTION_ID)).ThenCall(

		func(req *http.Request) (*http.Response, error) {
			body := testutils.RequestBodyToJson(t, req)
			assertConnectionTableConfigUpdateRequest(t, body)
			response := mock.NewResponse(req, http.StatusOK, prepareConnectionTableConfigUpdateResponse())
			return response, nil
		})
	...
	svc := ftClient.
		NewConnectionTableConfigUpdateService().
		Enabled(true).
		SyncMode("HISTORY").
	...
	assertConnectionTableConfigUpdateResponse(t, response)
}

func assertConnectionTableConfigUpdateRequest(t *testing.T, request map[string]interface{}) {
...
func assertConnectionTableConfigUpdateResponse(t *testing.T, response connections.ConnectionSchemaDetailsResponse) {
...
func prepareConnectionTableConfigUpdateResponse() string {
	return `{
    "code": "Success",
    "data": {
