...
func TestConnectionsListServiceDo(t *testing.T) {
	...
	handler := mockClient.When(http.MethodGet, "/v1/connections").
		ThenCall(func(req *http.Request) (*http.Response, error) {
			response := mock.NewResponse(req, http.StatusOK, prepareConnectionsListResponse())
			return response, nil
		})
...
func prepareConnectionsListResponse() string {
	return `{
  "code": "Success",
...
func assertConnectionsListResponse(t *testing.T, response connections.ConnectionsListResponse) {
	testutils.AssertEqual(t, response.Code, "Success")

	testutils.AssertEqual(t, len(response.Data.Items), 1)
	testutils.AssertEqual(t, response.Data.Items[0].ID, "connection_id")
